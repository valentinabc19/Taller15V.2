...
router.post('/ordenes', async (req, res) => {
    ...
    const items = req.body.items; // items es un arreglo que dice que productos se compraron y que cantidad por el ususario
    ...
    // Creamos la orden
    const response = await axios.get(`http://usuario:3001/usuarios/${usuario}`);
...
async function actualizarInventario(items) {
    for (const producto of items) {
        ...
        await axios.put(`http://producto:3002/productos/${producto.id}`, {
            nombre: nombre, 
            precio: precio,
            inventario: inv
        });
